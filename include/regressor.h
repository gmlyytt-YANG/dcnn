...
public:
  ...
  Regressor(const Regressor&);
	...
	const int stage);
  struct feature_node* GetGlobalBinaryFeatures(cv::Mat_<uchar>& image, cv::Mat_<double>& current_shape, BBox& bbox, cv::Mat_<double>& rotation, double scale);
	cv::Mat_<double> Predict(cv::Mat_<uchar>& image, cv::Mat_<double>& current_shape,
	BBox& bbox, cv::Mat_<double>& rotation, double scale);
...
public:
	...
	void Train(const std::vector<cv::Mat_<uchar> >& images,
		...
		Parameters& params);
	cv::Mat_<double> Predict(string& image_path, cv::Mat_<double>& current_shape, BBox& bbox, cv::Mat_<double>& ground_truth_shape);
	// cv::Mat_<double> Predict(cv::Mat_<uchar>& image, cv::Mat_<double>& current_shape, BoundingBox& bbox);
	cv::Mat_<double> Predict(string& image_path, cv::Mat_<double>& current_shape, BBox& bbox, int stage = -1, bool is_train = false);
