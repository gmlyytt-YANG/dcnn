...
bool RandomForest::TrainForest(//std::vector<cv::Mat_<double>>& regression_targets,
	...
	int step = floor(((double)augmented_images_index.size()) / trees_num_per_forest_);
	trees_.clear();
	all_leaf_nodes_ = 0;
	for (int i = 0; i < trees_num_per_forest_; i++){
		int start_index = std::max(i*step, 0);
		//int end_index = augmented_images_index.size() - (trees_num_per_forest_ - i - 1)*step;
		int end_index = std::min(i*step + floor(((double)step)/(1-overlap))-1,(double)(augmented_images_index.size()-1)) ;
...
cv::Mat_<double> RandomForest::GetBinaryFeatures(const cv::Mat_<double>& image,
	const BBox& bbox, const cv::Mat_<double>& current_shape, const cv::Mat_<double>& rotation, const double& scale){
