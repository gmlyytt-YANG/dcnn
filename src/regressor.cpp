...
void CascadeRegressor::Train(const std::vector<cv::Mat_<uchar> >& images,
	...
	std::vector<BBox> augmented_bboxes;
	...
	for (int i = 0; i < params_.regressor_stages_; i++){
		std::cout << std::endl <<"training stage: " << i << " of " << params_.regressor_stages_ << std::endl;
		...
		for (int j = 0; j < shape_increaments.size(); j++){
			...
			//std::cout << "augmented_current_shapes" << std::endl << augmented_current_shapes[j] << std::endl;
			//std::cout << "augmented_ground_truth_shapes" << std::endl << augmented_ground_truth_shapes[j] << std::endl;
...
std::vector<cv::Mat_<double> > Regressor::Train(const std::vector<cv::Mat_<uchar> >& images,
	...
	const std::vector<BBox>& augmented_bboxes,
  ...
  #pragma omp parallel for
	for (int i = 0; i < params_.landmarks_num_per_face_; ++i){
        ...
        rd_forests_[i].TrainForest(images,augmented_images_index, augmented_bboxes, augmented_current_shapes,
				rotations_, scales_);
	}
    ...
    for (int i = 0; i < augmented_current_shapes.size(); ++i){
      ...
      const BBox& bbox = augmented_bboxes[i];
...
cv::Mat_<double> CascadeRegressor::Predict(string& image_path,
	cv::Mat_<double>& current_shape, BBox& bbox, cv::Mat_<double>& ground_truth_shape){

    cv::Mat_<uchar> image = cv::imread(image_path, 0);
	cv::Mat_<uchar> tmp;
	image.copyTo(tmp);
...
cv::Mat_<double> CascadeRegressor::Predict(string& image_path,
	cv::Mat_<double>& current_shape, BBox& bbox, int stage, bool is_train){
    cv::Mat_<uchar> image = cv::imread(image_path, 0);
...
struct feature_node* Regressor::GetGlobalBinaryFeaturesMP(cv::Mat_<uchar>& image,
    cv::Mat_<double>& current_shape, BBox& bbox, cv::Mat_<double>& rotation, double scale){
    int index = 1;
...
void CascadeRegressor::LoadCascadeRegressor(std::string ModelName){
	std::ifstream fin;
	std::string model_dir = "../model/" + ModelName;
	
  fin.open((model_dir + "/" + ModelName + "_params.txt").c_str(), std::fstream::in);
	params_ = Parameters();
...
void CascadeRegressor::SaveCascadeRegressor(std::string ModelName){
  ...
  #elif __linux__
  struct stat st1 = {0};
  if (stat(model_dir.c_str(), &st1) == -1) {
    mkdir(model_dir.c_str(), 0777);
  }
  #endif	
  fout.open((model_dir + "/" + ModelName  + "_params.txt").c_str(), std::fstream::out);
...
void Regressor::LoadRegressor(std::string ModelName, int stage){
	char buffer[500];
  sprintf(buffer, "../model/%s/%s_%d_regressor.txt", ModelName.c_str(), ModelName.c_str(), stage);
	std::ifstream fin;
    ...
    sprintf(buffer, "../model/%s/%s_%d/%d_linear_x.txt", ModelName.c_str(), ModelName.c_str(), stage_, i);
		linear_model_x_.push_back(load_model(buffer));
    sprintf(buffer, "../model/%s/%s_%d/%d_linear_y.txt", ModelName.c_str(), ModelName.c_str(), stage_, i);
...
void Regressor::SaveRegressor(std::string ModelName, int stage){
	...
	assert(stage == stage_);
  sprintf(buffer, "%s/%s_%d_regressor.txt", model_dir.c_str(), ModelName.c_str(), stage);
	std::ofstream fout;
  ...
  for (int i = 0; i < linear_model_x_.size(); i++){
		sprintf(buffer, "%s/%s_%d", model_dir.c_str(), ModelName.c_str(), stage_);
    ...
    if (stat(buffer, &st) == -1) {
		...
		#endif
		//_mkdir(buffer);
    sprintf(buffer, "%s/%s_%d/%d_linear_x.txt", model_dir.c_str(), ModelName.c_str(), stage_, i);
		save_model(buffer, linear_model_x_[i]);

    sprintf(buffer, "%s/%s_%d/%d_linear_y.txt", model_dir.c_str(), ModelName.c_str(), stage_, i);
